...
public class DataString {
    ...
    String original2;   // Only used for terrain data
    String edited2; // Only used for terrain data
    String converted2;   // Only used for terrain data
    ...
    public DataString(String o, String e, String c){
        original = o;
        edited = e;
        converted = c;
        //skip = s;
    ...
    public DataString(String o, String o2, String e, String e2, String c){
        original = o;
        original2 = o2;
        edited = e;
        edited2 = e2;
    ...
    public void storeExtra(byte[] b){
        extra = new byte[b.length];
        
        for (int i = 0; i < extra.length; i++)
            extra[i] = b[i];
    }
    
    public void storeExtra2(byte[] b){
        extra2 = new byte[b.length];
    ...
    public byte[] getBytes(boolean use_converted){
        ...
        separator[0] = (byte) (0x81 & 0xff);    // '@' in SJIS
        separator[1] = (byte) (0x97 & 0xff);
    ...
    // USE THIS ONLY FOR THE LOCATION SECTION (the one that uses edited2)
    // Obtain the dialogue as an array of bytes (for insertion in a file)
    // If use_converted is true, we give the bytes of converted,
    // otherwise we give the ones of edited.
    public byte[] getBytes2(boolean use_converted){
        byte[] return_array = null;
        String chosen_text;
        
        if (use_converted)
            chosen_text = converted2;
        else
            chosen_text = edited2;        
    ...
    public void convertLetterPairs(){
        ...
        converted2 = ""; // Clean the converted text
        
        //String temp = "" + edited;
        
        String[] lines = edited.split("\n");
        String[] lines2 = edited2.split("\n");
        ...
        for (int i = 0; i < lines2.length; i++){
            if(lines2[i].length() % 2 != 0)  // Ensure the text has an even number of characters
                lines2[i] += " ";
            
            String temp_line = "";

            for (int j = 0; j < lines2[i].length(); j += 2){
                String pair = lines2[i].substring(j, j+2);
            ...
            converted2 += temp_line.trim();  // If there was an empty space in the end, we get rid of it
